    ...
    public static function fromDocument(
        ...
        if (trim((string) $request->header('If-None-Match'), '"') === $etag) {
            return response('', Response::HTTP_NOT_MODIFIED, [
                'ETag' => $etag,
                'X-HISSA-Contract-Version' => (string) config('integration-api.contract_version', '0.1.0'),
                'X-Request-ID' => (string) $request->attributes->get('integration_request_id', 'unknown'),
            ]);
        }

...
it('supports the same strong etag and conditional response for detail snapshot and export', function (): void {
    ...
    $detail = $this->get('/api/v1/filings/'.$filing->filing_id);
    $etag = $detail->headers->get('ETag');
    $snapshot = $this->get('/api/v1/snapshots/PUB-ETAG-1');
    $export = $this->get('/api/v1/filings/'.$filing->filing_id.'/export');
    $notModified = $this->withHeaders(['If-None-Match' => $etag])->get('/api/v1/filings/'.$filing->filing_id);

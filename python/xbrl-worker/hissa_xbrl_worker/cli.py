...
from .parser import parse_filing, build_failure_payload
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        log_event(logger, logging.INFO, "cli.invoked", "XBRL parser CLI invoked.")
        result = parse_filing(ParseRequest(Path(args.input), args.filing_id)); code = 0
    except (ValueError, SystemExit) as exc:
        if isinstance(exc, SystemExit) and exc.code == 0:
            raise
        log_event(logger, logging.ERROR, "cli.argument_error", "Invalid CLI arguments.", error_code="CLI_ARGUMENT_ERROR")
        result = build_failure_payload("", Path(""), "CLI_ARGUMENT_ERROR", "Invalid CLI arguments."); code = 2
    except WorkerError as exc:
        log_event(logger, logging.ERROR, "parser.failed", "XBRL parser execution failed.", error_code=exc.code, exit_code=exc.exit_code, duration_ms=round((time.perf_counter() - started) * 1000, 3))
